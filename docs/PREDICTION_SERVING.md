...
ScoutIQ stores Day 5 expected-points outputs in PostgreSQL so the backend API and future optimizer can consume deterministic prediction data. This layer does not add squad optimization, transfer optimization, or agent behavior.
...
The backend exposes typed JSON endpoints for future optimizer and frontend consumers:
...
Future optimizer work should read from `prediction_runs` and `player_predictions`, select an explicit run id or latest gameweek run, and keep that run id attached to any recommendation output. Double-gameweek aggregation should be an explicit optimizer concern rather than hidden in the serving loader.

The serving layer intentionally persists per-fixture prediction rows. It does not choose squads, propose transfers, or call LLM/agent workflows.
